...
using CASCExplorer;
using CascScraper.Schema;
using HeroInfoExtractor.Schema;
using HtmlAgilityPack;

namespace CascScraper {
    public class Scraper {
        private static readonly Dictionary<string, string> CooldownOnExpireAbilities = new Dictionary<string, string> {
        ...
        private readonly Dictionary<string, XmlDocument> _referenceCatalog = new Dictionary<string, XmlDocument>();

        public List<TalentInfo> TalentInfoList { get; private set; } = new List<TalentInfo>();
        public List<SkillInfo> SkillInfoList { get; private set; } = new List<SkillInfo>();
        public DataTable TalentInfoDataTable { get; set; }
        public int BuildNumber { get; private set; }
        public string BuildVersion { get; private set; }
        public Dictionary<string, ActorUnit> ActorUnits { get; } = new Dictionary<string, ActorUnit>();
        public Dictionary<string, byte[]> HeroImages { get; } = new Dictionary<string, byte[]>();
        ...
        private void PopulateActorUnits(XmlDocument cata) {
            ...
            foreach (XmlNode actorUnit in globalActorUnits) {
                ...
                if (heroIconPath != null) {
                    if (substitutions.ContainsKey(heroIconPath)) {
                        heroIconPath = substitutions[heroIconPath];
                    }
        ...
        public string GenerateTalentInfoSql() {
            ...
            using (var sqlWriter = new StringWriter()) {
                ...
                foreach (DataRow row in dt.Rows) {
                    ...
                    sqlWriter.Write(
                        $"('{hero}',{BuildNumber},{BuildNumber},{id},{level},'{name}','{description}')");
                    firstLine = false;
                }
        ...
        private void GenerateTalentInfoDataTable() {
            ...
            foreach (var t in TalentInfoList) {
                dt.Rows.Add(t.HeroName, t.TalentIdString, t.TalentId, t.TalentName, t.TalentDescription, t.Tier,
                    t.Column);
            }
        ...
        private void GetModHeroes() {
            ...
            foreach (var heroDir in heroesDirs) {
                ...
                var locStrings = locLines.Select(x => x.Split('=')).ToDictionary(x => x[0], x => string.Join("=", x.Skip(1)));

                var cfHeroBaseStormdata = (CASCFolder)cfHeroDir.GetEntry("base.stormdata");
                var cfConfig = (CASCFile)cfHeroBaseStormdata.GetEntry("GameData.xml");
                ...
                using (var f4 = GetFileStream(cfConfig)) {
                    ...
                    var heroName = catalog4.Catalog[0].Path.Substring(9); // Skip GameData/
                    var cfHeroGameData = (CASCFolder)cfHeroBaseStormdata.GetEntry("GameData");
                    var cfHero = (CASCFile)cfHeroGameData.GetEntry(heroName);
                    if (cfHero == null) {
                        // Not a hero
                        continue;
                    }
            ...
            foreach (var theHero in catalog.CHero.Where(x => x.TalentTreeArray != null)) {
                ...
                switch (heroName) {
                    ...
                    default: {
                        ...
                        if (ActorUnits.ContainsKey(actorKey)) {
                            var heroImage = ActorUnits[actorKey].HeroIcon ??
                                            GetImage($@"Assets\Textures\ui_targetportrait_hero_{theHero.id.ToLower()}.dds");
                            HeroImages[heroName] = heroImage;
                        }
                        else {
                            HeroImages[heroName] = GetImage($@"Assets\Textures\ui_targetportrait_hero_{theHero.id.ToLower()}.dds");
                        }
                ...
                foreach (var talent in talentTree) {
                    ...
                    if (talentDic.ContainsKey(talentKey)) {
                        talentEntry = talentDic[talentKey];
                        string iconKey;
                        if (talentEntry.Face.Count > 0) {
                            iconKey = talentEntry.Face[0].value;
                        }
                        else if (_talentGenericDic.ContainsKey(talentKey)) {
                            talentEntry = _talentGenericDic[talentKey];
                            iconKey = talentEntry.Face[0].value;
                        }
                        else {
                    ...
                    TalentInfoList.Add(new TalentInfo {
                        TalentId = talentId++,
                    ...
                    if (talentEntry.Active.Any(x => x.value == "1") && talentEntry.Abil.Count > 0) {
                        ...
                        var heroic = xdoc.SelectSingleNode($"//HeroAbilArray[@Abil='{abilKey}']/Flags[@index='Heroic']");
                        if (heroic != null) {
                            continue;
                        }
                ...
                foreach (var abil in abilities) {
                    ...
                    if (buttonKey == null || !buttonDic.ContainsKey(buttonKey)) {
                        Console.WriteLine($"WARNING: Hero {heroName}, couldn't find button key {buttonKey} for ability {abil.Abil}");
                        continue;
                    }
                    ...
                    if (xLayout is null) {
                        if (abil.Abil == "GallTalentEyeOfKilrogg" ||
                            abil.Abil == "NecromancerBoneArmor") {
        ...
        private int GetCooldown(XmlNode xdoc, string abilKey) {
            if (abilKey == "HeroGenericSpellShield") {
                ...
                var key = "HeroGenericSpellShieldApplyPlayerCooldown";
            ...
            if (CooldownOnExpireAbilities.ContainsKey(abilKey)) {
                var key = CooldownOnExpireAbilities[abilKey];
                var xCooldown = xdoc.SelectSingleNode($"//CEffectModifyUnit[@id='{key}']/Cost");
                var cdMaybe = xCooldown?.Attributes?["CooldownTimeUse"]?.Value;
                if (cdMaybe == null) {
                    var xCooldown2 = xdoc.SelectSingleNode($"//CEffectModifyPlayer[@id='{key}']/Cost");
                    if (xCooldown2 != null) {
                        var xTimeUse = xCooldown2.SelectSingleNode("./Cooldown/TimeUse").Attributes["value"].Value;
                        return int.Parse(xTimeUse);
                    }
                ...
                var xAbilEffect = xdoc.SelectSingleNode($"//CAbilEffectInstant[@id='{key}']/Cost/Cooldown");
                if (xAbilEffect != null) {
        ...
        private bool IsDeathwingWorldBreakerAbility(CatalogCHeroHeroAbilArray abil) {
            if (abil.Abil == "DeathwingLavaBurst" ||
                abil.Abil == "DeathwingEarthShatter") return true;
            return false;
        }

        private byte[] GetImage(string cascPath) {
            var ddsPath = cascPath.Substring(16); // Skip Assets/Textures/
            var cfIcon = (CASCFile)_cfTextures.GetEntry(ddsPath);
            if (cfIcon == null) {
                        ...
                        if (nodeToRemove.HasChildNodes) {
                            ...
                            for (var i = 1; i < nodeToRemove.ChildNodes.Count; i++) {
                                cursor = (nodeToRemove.ParentNode ?? doc.DocumentNode).InsertAfter(
                                    nodeToRemove.ChildNodes[i], cursor);
                            }
                        }
                        else {
            ...
            if (dNodes != null) {
                foreach (var dNode in dNodes) {
                    ...
                    if (gref != null) {
                        ...
                        var fmt = "0." + new string(Enumerable.Range(0, prec).Select(x => '#').ToArray());
        ...
        private static string FixGref(string gref) {
            ...
            while (idx != -1) {
                ...
                gref = gref.Substring(0, idx) +
                       gref.Substring(idx + 8, (closingApostrophe - idx - 8)) +
                       gref.Substring(closingBrack + 1);
        ...
        private void GetBaseHeroes() {
            ...
            foreach (var heroDir in heroesDirs) {
                ...
                var cfHero = (CASCFile)cfHeroDir.GetEntry($"{heroDirBare}.xml");
                if (cfHero == null) {
                    // Not a hero
                    continue;
                }
        ...
        private void OpenCasc() {
            ...
            var versions = cfVersions.Entries.Values.Select(x => int.Parse(x.Name.Substring(4))).ToList();
        ...
        public (CASCHandler casc, CASCFolder fldr) OpenStorage(string arg) {
            ...
            using (var _ = new PerfCounter("LoadListFile()")) {
                ...
                bgWorker.ProgressChanged += (s, e) => Console.WriteLine($"{e.ProgressPercentage} {e.UserState}");
                bgWorker.RunWorkerCompleted += (s, e) => ev.Set();
                bgWorker.DoWork += (s, e) => {
        ...
        private List<string> GetDirectories(CASCFolder cascFolder) {
