...
namespace CascScraper.Schema {
    class ExprVisitor : arithmeticBaseVisitor<ResultType> {
        ...
        private string ToCtype(string cobjName) {
            var idx = cobjName.IndexOfAny("ABCDEFGHIJKLMNOPQRSTUVWXYZ".ToArray(), 2);
            return idx == -1 ? cobjName.Substring(1) : cobjName.Substring(1, idx - 1);
        }
        ...
        private XmlNode FindObjectCatalog(XmlDocument doc, string ctype, string cid) {
